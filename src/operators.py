...
OPERATORS = {
    '+': Operator(OperatorType.BINARY, Associativity.ASSOCIATIVE, 0, lambda x, y: x + y),
    '-': Operator(OperatorType.BINARY, Associativity.LEFT_ASSOCIATIVE, 0, lambda x, y: x - y),
    'unary-': Operator(OperatorType.PREFIX, Associativity.NONE, 1, lambda x: -x),
    '*': Operator(OperatorType.BINARY, Associativity.ASSOCIATIVE, 1, lambda x, y: x * y),
    '/': Operator(OperatorType.BINARY, Associativity.LEFT_ASSOCIATIVE, 1, lambda x, y: x / y),
    '^': Operator(OperatorType.BINARY, Associativity.RIGHT_ASSOCIATIVE, 2, lambda x, y: x ** y),
    'sqrt': Operator(OperatorType.PREFIX, Associativity.NONE, 3, math.sqrt),
    'cbrt': Operator(OperatorType.PREFIX, Associativity.NONE, 3, math.cbrt),
